...
import se.jknowledge.transcribe.api.model.openai.ChatMessageRequest;
...
public class PromptBuilder {
    ...
    public String appendSlideLinks(String answer, List<ChunkMatch> matches, String slideBaseUrl) {
        String suffix = buildSlideLinksMarkdown(matches, slideBaseUrl);
        if (suffix.isBlank()) {
            return answer;
        }

        String base = answer == null ? "" : answer.strip();
        if (base.isBlank()) {
            return suffix.strip();
        }
        return base + suffix;
    ...
    private String buildSlideLinksMarkdown(List<ChunkMatch> matches, String slideBaseUrl) {
        Set<String> seen = new LinkedHashSet<>();
        List<String> lines = new ArrayList<>();

        for (ChunkMatch match : matches) {
            if (match.imagePath() == null || match.imagePath().isBlank()) {
                continue;
            }

            String filename = java.nio.file.Path.of(match.imagePath()).getFileName().toString();
            if (!seen.add(filename)) {
                continue;
            }

            String presentationName = firstNonBlank(match.presentationId(), match.videoFile(), "presentation");
            String label = match.slideIndex() != null
                    ? presentationName + " slide " + match.slideIndex()
                    : presentationName;
            lines.add("- [" + label + "](" + slideBaseUrl + encodePathSegment(filename) + ")");
        }
